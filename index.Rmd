...
data.tt2020 <- read_csv("~/R/Tong dieu tra Nong Lam Thuy San 2020/Thoa Notes/Data_TrangTrai_2020.csv")
head(data.tt2020)

## Summary Statistics 

summary(data.tt2020)
summary(data.tt2020[c("PHANHH","PHANHH01_DT","PHANHH01_KL")])
summary(data.tt2020[c("LDSXNU_TS","PHANHH01_DT","PHANHH01_KL")])

women <- data.tt2020 %>% 
  filter(GIOITINH == 2)

cor(data.tt2020[c("PHANHH01_DL", "PHANHH01_KL")])
mean(~PHANHH01_KL, data=data.tt2020, na.rm=TRUE)
sd(~PHANHH01_KL, data=data.tt2020, na.rm=TRUE)
favstats(~PHANHH01_KL, data=data.tt2020)

## Group by age:

data.tt2020 %>% 
  group_by("NAMSINH") %>% 
         summarise(yr_min=min(NAMSINH) , yr_max=max(NAMSINH))

## Relationship between quantity of fertilzer use on annual crops
...
ggplot(data = data.tt2020,
       mapping = aes(x = PHANHH01_DT,
                     y = PHANHH01_KL))+
  geom_point(method = "loess", se = FALSE)+
  scale_x_log10() +
